...
import com.example.myapplication.view.BatteryMeasurementView
...
class ChargingChartActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val barChart = findViewById<BatteryMeasurementView>(R.id.barChartView)
        val sampleData = List(96) { index ->
            val hour = index / 4
            val minute = (index % 4) * 15
            val timeStr = String.format("%02d:%02d", hour, minute)
            val value = if (index in 20..25) (5..9).random() else (15..80).random()
            val extra = if (index in 20..26 || index in 50..60 || index in 90..96 || index in 72..74) 1 else null
            ChartData(
                indexName = "Test",
                time = timeStr,
                indexValue = value,
